<script>
    ...
    const defaultKeywords = { keywords: "final", version: "0.3.4" };
    let settings = { version: "0.3.4", zeros: "no", developer: "off", mode: "default", keywords: "final", edited: false };
    //export const settings = writable('aeries-grades+-settings', { version: "0.3.4", zeros: "no", developer: "off", mode: "default", keywords: "final", edited: false });
...
{#if current == "Gradebook"}
    <Periods started={started} {defaultKeywords} bind:settings></Periods>
{:else if current == "Gradebook Details"}
    <Period started={started} bind:settings></Period>
{/if}