...
<div class="p-8 {settings.mode == 'dark' ? "bg-zinc-900 text-white" : settings.mode == 'light' ? "bg-zinc-100 text-black" : "bg-zinc-100 dark:bg-zinc-900 text-black dark:text-white"}  h-full w-full relative">
    ...
    {#if settingsOpen}
        ...
        <div transition:fade={{ duration: 100 }} class="absolute z-[5] p-6 {settings.mode == 'dark' ? "bg-zinc-900" : settings.mode == 'light' ? "bg-zinc-100" : "bg-zinc-100 dark:bg-zinc-900"} rounded-3xl max-w-[calc(100%-2rem)] w-[36rem] min-h-[24rem] max-h-[calc(100%-6rem)] left-1/2 top-10 -translate-x-1/2 overflow-auto">
            ...
            <div class="right-0 top-0 font-bold text-base p-4 mb-3 rounded-md bg-red-500 bg-opacity-20 text-red-500">
                Due to bugs with saving settings, settings will not be able to save temporarily.
            </div>

            <div class="flex items-center justify-between relative">
                <p class="text-lg">Appearance</p>

                <select bind:value={settings.mode} disabled name="mode" id="mode" class="disabled:cursor-not-allowed {settings.mode == 'dark' ? "text-white bg-zinc-100 bg-opacity-10" : settings.mode == 'light' ? "text-black bg-zinc-900 bg-opacity-10" : "text-black dark:text-white bg-zinc-900 dark:bg-zinc-100 bg-opacity-10 dark:bg-opacity-10"} rounded-md text-lg px-3 pr-8 py-1.5  bg-none w-28">
            ...
            <div class="flex items-center justify-between relative mt-2">
                ...
                <select bind:value={settings.zeros} disabled name="zeros" id="zeros" class="disabled:cursor-not-allowed {settings.mode == 'dark' ? "text-white bg-zinc-100 bg-opacity-10" : settings.mode == 'light' ? "text-black bg-zinc-900 bg-opacity-10" : "text-black dark:text-white bg-zinc-900 dark:bg-zinc-100 bg-opacity-10 dark:bg-opacity-10"} rounded-md text-lg px-3 pr-8 py-1.5  bg-none w-28">
            ...
            <div class="flex items-center justify-between relative mt-2">
                ...
                <select bind:value={settings.developer} disabled name="developer" id="developer" class="disabled:cursor-not-allowed {settings.mode == 'dark' ? "text-white bg-zinc-100 bg-opacity-10" : settings.mode == 'light' ? "text-black bg-zinc-900 bg-opacity-10" : "text-black dark:text-white bg-zinc-900 dark:bg-zinc-100 bg-opacity-10 dark:bg-opacity-10"} rounded-md text-lg px-3 pr-8 py-1.5  bg-none w-28">
            ...
            <div class="relative mt-3">
                ...
                <textarea bind:value={settings.keywords} disabled class="disabled:cursor-not-allowed w-full min-h-[8rem] rounded-lg p-3 {settings.mode == 'dark' ? "bg-zinc-100 bg-opacity-10" : settings.mode == 'light' ? "bg-zinc-900 bg-opacity-10" : "bg-zinc-900 bg-opacity-10 dark:bg-zinc-100 dark:bg-opacity-10"}"></textarea>
            ...
            <p class="mt-2 text-right text-sm opacity-75">Version 0.3.4</p>
